...
export default function ApproveDemoPage() {
  ...
  const handleAction = async (actionType: 'approve' | 'reject') => {
    ...
    try {
      ...
      setMessage(
        actionType === 'approve'
          ? 'Demo approved. They will receive their login credentials via email.'
  ...
  return (
    <>
      <Helmet>
        <title>Approve Demo | Custom Booking</title>
      </Helmet>
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <div className="w-full max-w-md bg-card rounded-xl border border-border shadow-sm p-6">
          {status === 'form' && (
            <>
              <h1 className="text-xl font-semibold text-foreground mb-2">Approve Demo Access</h1>
              <p className="text-muted-foreground text-sm mb-6">
                Someone requested a demo from the landing page. Approving will create their account and send them their login credentials.
              </p>
